...
const Board = () => {
	...
	return (
		<div className={styles.container}>
			{modalState.active && (
				<Modal modal={modalState} setModal={setModalState} />
			)}
			<Heading order={sortOrder} setOrder={setSortOrder} />
			<div className={styles.row}>
				<DragDropContext
					onDragEnd={(result) =>
						onDragEnd(result, listsState, setListsState, cardUpdate)
					}
				>
					{listsState.length <= 0 ? (
						<BoardPlug />
											...
											{(provided, snapshot) => {
												return (
													...
													>
														...
														{list.cards
															.sort((x, y) => orderSort(x, y, sortOrder))
															.map((card, index) => {
